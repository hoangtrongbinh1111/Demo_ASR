...
function createDownloadLink(blob,encoding) {
    ...
    xhr.onload=function(e) {
        if(this.readyState === 4) {
			console.log("Server returneddemo: ",e.target.responseText);
...
function handleUploadFile()
{
	...
	const file=new Audio("/tmp/330.wav");
	console.log(file);
	var au = document.createElement('audio');
	var li = document.createElement('li');
	//add controls to the <audio> element
	...
	// var lstAud=['330.wav','331.wav','332.wav'];
	// for(let i=0;i<lstAud.length();i++){
	// 	var fd=new FormData();
	// 	fd.append("the_file",lstAud[i]);
	// 	xhr.open("POST","/recog",true);
	// 	xhr.send(fd);
	// }
	var fd=new FormData();
